...
def process_song_data(spark, input_data, output_data):
    ...
    song_data = input_data + "song-data/A/B/C/*.json" #Need to change
    ...
    songs_table.write.partitionBy("year", "artist_id").mode('overwrite').parquet("songs.parquet") #Need to change
    ...
    artists_table.write.mode("overwrite").parquet("artists.parquet") #Need to change
...
def process_log_data(spark, input_data, output_data):
    ...
    log_data = "s3a://udacity-dend/log_data/2018/11/2018-11-12-events.json" #Need change
    ...
    users_table.write.mode("overwrite").parquet("users.parquet") #Need to change
    ...
    time_table.write.partitionBy('year','month').parquet("time.parquet",mode="overwrite") #Need to change
    ...
    songplays_table.write.partitionBy("year", "month").mode('overwrite').parquet("songplays.parquet")#Need to change
